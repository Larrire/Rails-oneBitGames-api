  ...
  context "when #call" do
    ...
    context "when params are present" do
      let!(:search_system_requirements) do
        system_requirements = []
        15.times { |n| system_requirements << create(:system_requirement, name: "Search #{n + 1}", video_board: "GeForce #{n + 1}") }
        system_requirements
      end
      ...
      it "does not return unexpected records" do
        ...
        service = described_class.new(SystemRequirement.all, params)
        service.call
        expect(service.records).to_not contain_exactly *unexpected_system_requirements
      end
    end
