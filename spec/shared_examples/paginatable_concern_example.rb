shared_examples "paginatable concern" do |factory_name|
  ...
  context "when records fits page size" do
    ...
    context "when :page is fullfilled and :length is empty" do
      let(:page) { 2 }
    ...
    context "when :page and :length are fullfilled and fits records size" do
      let(:page) { 2 }
      let(:length) { 5 }
      ...
      it "returns the right records from the right page" do
        paginated_records = described_class.paginate(page, length)
        first_record_index = 5
        last_record_index = 9
        expected_records = described_class.all[first_record_index..last_record_index]
        expect(paginated_records).eq expected_records
      end
    end
  
    context "when :page and :length are fullfilled and does not fit records size" do
      let(:page) { 2 }
      let(:length) { 30 }
  ...
  end # context "when records fits page size"
  ...
  context "when records does not fit page size" do
    ...
    context "when :page and :length are fullfilled" do
      let(:page) { 2 }
      let(:length) { 5 }
  ...
  end # context "when records does not fit page size"
